...
//------------------ END MODULE SCOPE VARIABLES --------------------------
  function init($append_target) {
    // Add stuff we need
    //$('<div id="terminal" class="terminal"></div>').appendTo(".editor--terminal .editor__canvass");
    //$('<textarea id="terminalfocus" class="terminal__focus" rows="1" cols="1"></textarea>').appendTo(document.body);

    //$(".editor--terminal .editor__canvass").append('<div id="terminal" class="terminal"></div>');
    //$('<textarea id="terminalfocus" class="terminal__focus" rows="1" cols="1"></textarea>').appendTo(".editor--terminal .editor__canvass");

    $append_target.append('<div id="terminal" class="terminal"></div>');
    $append_target.append('<textarea id="terminalfocus" class="terminal__focus" rows="1" cols="1"></textarea>');
...
//      callback(data);
//    });
    updateTerminalLine('PrjX-Beans Terminal V0.2', true);
  }

  var clearTerminal = function () {
    // Get just the last entered line
    var currentLine = v100.Terminal.getInputLine();
    ...
    v100.callProcessor("terminalClear");
  };
  ...
  var updateTerminal = function () {
    ...
    for (var y in termText) {
      var line = termText[y];
      if (y == termCursorY) {
        var ch = v100.Utils.getSubString(line, termCursorX, 1);
        line = v100.Utils.escapeHTML(
          v100.Utils.getSubString(line, 0, termCursorX)) +
               "<span class='terminal__cursor'>" + v100.Utils.escapeHTML(ch) + "</span>" +
               v100.Utils.escapeHTML(v100.Utils.getSubString(line, termCursorX + 1));
      } else
        line = v100.Utils.escapeHTML(line);
    ...
    if (elements[termCursorY] !== undefined) {
      if (termCursorY < 14) {
        //elements[0][0].scrollIntoView();
      }
      else {
        elements[termCursorY-14][0].scrollIntoView();
      }
    ...
    if (termControlChars.length == 0) {
      switch (ch) {
        ...
        { // carriage return
          ...
          v100.Utils.socket.emit('my other event', { my: tenm}); // Replace HTTP with Socket.io
          $.gevent.publish('e-v100-text-enter',{ enter_text: tenm });
        ...
        default :
        {
          ...
          termText[termCursorY] = trimRight(
            v100.Utils.getSubString(termText[termCursorY], 0, termCursorX) +
            String.fromCharCode(ch) +
            v100.Utils.getSubString(termText[termCursorY], termCursorX + 1));
          termCursorX++;
        }
      }
...
// ----------------------------------------------------------------------------------------------------------
  ...
  function grabSerialPort() {
    ...
    v100.Terminal.setInputDataHandler(function (data) {
...
// prjX.Core.Utils.socket.init('http://localhost');
  v100.Utils.socket.on('news', function (data) {
    console.log(data);
