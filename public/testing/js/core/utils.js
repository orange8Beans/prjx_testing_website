...
prjXX.Core.Utils = (function(){
  ...
  function getprjXPrompt(callback) {
    ...
    var prevReader = prjXX.Core.Serial.startListening(function (readData) {
    ...
    var nextStep = function() {
      ...
      prjXX.Core.Serial.startListening(prevReader);
      // call our callback
      callback();
    };
    // send a newline, and we hope we'll see '=undefined\r\n>'
    prjXX.Core.Serial.write('\n');
  };  
  ...
  function executeExpression(expressionToExecute, callback) {
    ...
    if(prjXX.Core.Serial.isConnected()){
      prjXX.Core.Utils.getprjXPrompt(function() {
  ...
  function htmlTable(obj) {
    var html = '<table>';
    for (var key in obj) {
      html += '<tr><th>'+prjXX.Core.Utils.escapeHTML(key)+'</th><td>'+prjXX.Core.Utils.escapeHTML(obj[key])+'</td></tr>';
    }
    return html + '</table>';
  }
